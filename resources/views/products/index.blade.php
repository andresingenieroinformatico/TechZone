                                ...
                                <div class="premium-card group">
                                    ...
                                    <div class="p-6">
                                        <div class="flex justify-between items-start mb-2">
                                            <span class="text-xs font-medium text-blue-600 dark:text-blue-400 uppercase tracking-wider">{{ $product->category->name }}</span>
                                            <div class="flex items-center">
                                                ...
                                                <span class="ml-1 text-sm font-bold text-gray-600 dark:text-gray-400">{{ number_format($product->average_rating, 1) }}</span>
                                            </div>
                                        </div>
                                        <a href="{{ route('products.show', $product->slug) }}" class="block mb-4">
                                            <h2 class="text-lg font-bold text-gray-900 dark:text-white group-hover:text-blue-600 transition-colors line-clamp-1">{{ $product->name }}</h2>
                                        </a>
                                        <div class="flex items-center justify-between">
                                            <span class="text-2xl font-black text-gray-900 dark:text-white">{{ $product->formatted_price }}</span>
                                            <form action="{{ route('cart.store') }}" method="POST">
                                                @csrf
                                                <input type="hidden" name="product_id" value="{{ $product->id }}">
                                                <input type="hidden" name="quantity" value="1">
                                                <button type="submit" class="p-2.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-xl hover:bg-blue-600 hover:text-white transition-all duration-300">
